...
真正在线翻译时，请使用自己有权处理的 OCR 文本和自己的 DeepSeek key。自动检查中的“译：”文本只是管线测试，**不是真实译文**，不能作为书籍成品。PDF/EPUB 还要求系统安装 Pandoc、XeLaTeX、中文字体和 EPUBCheck。Windows EXE 的冻结版离线检查由 GitHub Actions 运行；实际付费 API 调用和 PDF/EPUB 出版工具不在公开 CI 中执行。
